...
export function Header({lang,setLang,onboarding=false}:{lang:Language;setLang:(l:Language)=>void;onboarding?:boolean}) {
  const en = lang === 'en';
  return <header className="header"><div className="container header-inner"><a className="brand" href={`/?lang=${lang}`} aria-label="Codeflow Studios"><img src="/codeflow-logo.png" alt="Codeflow Studios" width="200" height="200" /></a><nav aria-label={en?'Main navigation':'Hoofdnavigatie'}>{!onboarding && <><a href={`/?lang=${lang}#services`}>{en?'Services':'Diensten'}</a><a href={`/services/rebranding?lang=${lang}`}>Rebranding</a><a href={`/services/software-development?lang=${lang}#game`}>Game</a><a href={`/?lang=${lang}#contact`}>Contact</a></>}<div className="language" role="group" aria-label={en?'Language':'Taal'}><button onClick={()=>setLang('nl')} aria-pressed={lang==='nl'}>NL</button><span>/</span><button onClick={()=>setLang('en')} aria-pressed={lang==='en'}>EN</button></div>{onboarding?<a className="nav-action" href={`/?lang=${lang}`}>{en?'Back to website':'Naar de website'} <ArrowUpRight size={17}/></a>:<a className="nav-action" href={`/onboarding?plan=flow&lang=${lang}`}>{en?'Explore your flow':'Ontdek jouw flow'} <ArrowUpRight size={17}/></a>}</nav></div></header>;
}
export function Footer({lang}:{lang:Language}) {
 const en=lang==='en';return <footer className="footer container"><div className="footer-top"><span className="footer-name">Codeflow<span> Studios</span></span><a href="#top">{en?'Back to top':'Terug naar boven'} <ArrowUpRight size={18}/></a></div><div className="footer-bottom"><span>© 2026 Codeflow Studios CommV</span><span>{en?'Bruges, Belgium':'Brugge, België'}</span><span>codeflowstudios.be</span></div><p className="domain-list">codeflowstudios.net <span>·</span> codeflowstudios.org <span>·</span> codeflowstudios.eu</p></footer>;
}
